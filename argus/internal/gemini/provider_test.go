...
func request() agent.ModelRequest {
	return agent.ModelRequest{
		...
		Messages: []agent.Message{
			...
			{Role: agent.RoleUser, Parts: []agent.MessagePart{{Text: &agent.TextPart{Text: "fresh screenshot"}}, {Image: &agent.ImagePart{Data: []byte("new-png"), MediaType: "image/png"}}}},
		},
...
func assertPayload(t *testing.T, got map[string]any) {
	...
	want := map[string]any{
		...
		"contents": []any{
			...
			map[string]any{"role": "user", "parts": []any{map[string]any{"text": "fresh screenshot"}, map[string]any{"inlineData": map[string]any{"mimeType": "image/png", "data": "bmV3LXBuZw=="}}}},
		},
